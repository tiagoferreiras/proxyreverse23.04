...
app.get('/contatos/:parametro', (req, res) => {
  ...
  if (!isNaN(parametro)) {
    ...
    const result = connection.query('SELECT nome, telefone, email, logradouro, numero, complemento, bairro, cidade, uf, cep, t.tipo as tipo_cliente FROM cliente c inner join tipo_cliente t on t.id = c.tipo_cliente_id WHERE c.id = ?', [id], (err, rows) => {
      if (err) {
